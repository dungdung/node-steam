...
exports.emitType = function(sym) {
  if (sym instanceof symbol_locator.WeakSymbol) {
    ...
    if (!sym.prop) {
      ...
      // this is a hack, should use already generated value
      return exports.emitType(sym.prop.default[0]);
    }
  }
      ...
      if (defflags == 'proto') {
        ...
        if (prop.flagsOpt) {
          ...
          if (~['long', 'ulong'].indexOf(exports.emitType(prop.type))) {
            ctor = ByteBuffer.Long.isLong(object[symname]) ? object[symname] : ByteBuffer.Long.fromString(object[symname] || '0');
          } else {
            ctor = object[symname] || 0;
          }
        }
      } else if (~['long', 'ulong'].indexOf(exports.emitType(prop.type))) {
        ctor = ByteBuffer.Long.isLong(object[symname]) ?
          object[symname]
          : ByteBuffer.Long.fromString(object[symname] || (ctor == 'ulong.MaxValue' ? '18446744073709551615' : ctor));
      } else {
...
function emitClassDecoder(cnode) {
  Steam.Internal[cnode.name].decode = function(buffer) {
    ...
    cnode.childNodes.forEach(function(prop) {
      ...
      if (!size) {
        // assume protobuf
        // does this need an explanation? TODO
